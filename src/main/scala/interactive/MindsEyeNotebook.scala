...
abstract class MindsEyeNotebook(server: StreamNanoHTTPD, log: HtmlNotebookOutput with ScalaNotebookOutput) {
  ...
  val monitor = new TrainingMonitor {
    ...
    override def onStepComplete(currentPoint: IterativeTrainer.Step): Unit = {
      ...
      if(shouldReplotMetrics(iteration)) regenerateLog()
  ...
  def regenerateLog() = {
    val file = log.file("../metricsHistory.html")
    val report = new HtmlNotebookOutput(log.workingDir, file) with ScalaNotebookOutput
    generateLog(report).andThen({case _⇒report.close()})
  }
  def generateLog(log: ScalaNotebookOutput = log): Future[Unit] = {
    if(!history.isEmpty) {
      ...
      Future {
        ...
        keys
          ...
          .foreach(key⇒{
            ...
            try {
              log.eval {
                val plot: PlotCanvas = ScatterPlot.plot(dataAsScala.map(row ⇒ Array[Double](
